...
void setup() {
  noSmooth();
  frameRate(15);
  size(1200, 700, P2D);//P2D is important
  background(0);
  stroke(255);
  textSize(40);
  text("loading FSCMd...", width*.4, height*.4);
  s=new Client(this, "localhost", 12340);
  HDD=new fscmdHeadingDistanceDisplay(115, 195, 200, maxDispFlyDistMeters);
  OTD=new fscmdOrientationDisplay(500, 0, 700, maxDispFlyDistMeters);
  MBGBOO=new fscmdMiBarGraphDisplay(0, 0, 55, 3, "overall BNO055 orientation calibration status");
  MBGBOA=new fscmdMiBarGraphDisplay(65, 0, 40, 3, "BNO055 accel status");
  MBGBOG=new fscmdMiBarGraphDisplay(110, 0, 40, 3, "BNO055 gyro status");
  MBGBOM=new fscmdMiBarGraphDisplay(155, 0, 40, 3, "BNO055 mag status");
  PWG=new fscmdPowerGraphDisplay(67, 300, 432, 100, 3.0, 4.2, 3000);
  PWRCBG=new fscmdCoBarGraphDisplay(PWG.posx-66, PWG.posy, PWG.sizey, PWG.minval, PWG.maxval, 3.3, "BAT V");
  FSCMDRSSI=new fscmdCoBarGraphDisplay(200, 2, 80, -100, -15, -80, "FSCM");
  TRANSRSSI=new fscmdCoBarGraphDisplay(270, 2, 80, -100, -15, -80, "Trans");
  //  TBATCGD=new fscmdCoBarGraphDisplay(370, 0, 80, 3.3, 6, 3.5, "T Bat");
  BSH=new fscmdButton(425, 210, 55, color(0, 150, 0), true, "set home");
...
void draw() {
  ...
  fscmdDisplayInfo(dispMsg, dispVal, 0, 424, 500, 275, 10);
